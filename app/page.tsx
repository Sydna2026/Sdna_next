...
// Fallback icon for admin-added specializations that aren't in the map above.
const FALLBACK_ICON = (
...
function guidelineItems(guidelines: GuidelineContent[]): GuidelineItem[] {
  return guidelines.map((g) => ({
    ...
    desc: g.desc,
    details: g.details,
    icon: ICON_BY_SLUG[g.slug] ?? FALLBACK_ICON,
  }));
}
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-md flex items-center justify-center p-4 z-50 animate-fadeIn">
          <div className="bg-[#F9ECE4] rounded-[24px] border-2 border-[#A08C8A] max-w-xl w-full p-6 sm:p-8 shadow-2xl relative text-[#4A4A4A] max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-6">
              ...
              <div>
                <h4 className="text-xs font-black text-[#A08C8A] uppercase tracking-widest mb-3">References</h4>
                <ul className="space-y-3">
                  {selectedGuideline.details.map((detail, index) => (
                    <li key={index} className="flex items-start space-x-3 text-sm text-gray-700">
                      <svg className="w-5 h-5 text-[#A08C8A] flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13.828 10.172a4 4 0 015.656 0 4 4 0 010 5.656l-3 3a4 4 0 01-5.656-5.656M10.172 13.828a4 4 0 01-5.656 0 4 4 0 010-5.656l3-3a4 4 0 015.656 5.656" /></svg>
                      {/^https?:\/\//i.test(detail.trim()) ? (
                        <a href={detail.trim()} target="_blank" rel="noopener noreferrer" className="leading-relaxed text-[#A08C8A] break-words hover:underline">{detail.trim()}</a>
                      ) : (
